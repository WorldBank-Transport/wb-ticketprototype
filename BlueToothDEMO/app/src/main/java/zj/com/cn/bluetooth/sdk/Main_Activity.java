...
/****************************************************************************************************/
	 ...
	 * 连接成功后打印测试页
	 */
	private void Print_Test(String text){
	...
	private byte[] createHash(byte[] passcode, int prime) {
		// the hash, when xor'd with the passcode, should be divisible by 2^(len(passcode) / 2)
		byte[] b = new byte[passcode.length / 8];
		new Random().nextBytes(b);
		BigInteger bigint = new BigInteger(b);
		bigint = bigint.multiply(new BigInteger(Integer.toString(prime)));
		byte[] multiplied = bigint.toByteArray();
		byte[] finalHash = new byte[multiplied.length];
		for (int i = 0; i < multiplied.length; i++) {
			finalHash[i] = (i < passcode.length) ? (byte)(multiplied[i] ^ passcode[i]) : multiplied[i];
		}
		return finalHash;
	}
...
/************************************************************************************************/	
    ...
    private void createImage() {
            Print_Test("World Bank Test Ticket #" + new Random().nextInt(10000));
	        try {
	            // 需要引入zxing包
	            ...
	            String text = createHash(b, 78787).toString();
